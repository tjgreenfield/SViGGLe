...
namespace SVGL
{
    namespace Elements
    {
        ...
        void Group::appendChild(XML::Node_uptr&& child)
        {
            ...
            else if (auto styledChild = dynamic_cast<Styled*>(child.get()))
            {
                child.release();
                styledChildren.emplace_back(styledChild);
            }
        ...
        Root* Group::findElementByID(const char* _id)
        {
            ...
            {
                for (const Styled_uptr& child : styledChildren)
                {
        ...
        {
            Styled::cascadeStyle(styleSheetIndex);
            for (const auto& i : styledChildren)
            {
            ...
            group(_group)
        {
            ...
            for (const auto& i : group->styledChildren)
            {
