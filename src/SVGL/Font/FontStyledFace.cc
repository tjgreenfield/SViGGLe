...
namespace SVGL
{
    namespace Font
    {
        ...
        void StyledFace::buffer(const Styles::Vector& style, const char* text, double tolerance)
        {
            while (*text)
            {
                if (glyphBuffers.count(*text) == 0)
                {
                    ...
                    glyphBuffers[*text].buffer(glyph->getCommandSet(), style, tolerance * face->ftFace->units_per_EM);
                }
                ++text;
            }
        }

        void StyledFace::render(Render::Context* context, const Styles::Vector& style, const char* text) const
        {
